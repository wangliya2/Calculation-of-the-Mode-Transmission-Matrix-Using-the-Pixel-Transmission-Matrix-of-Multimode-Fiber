...
def _solve_u_by_sign_change(l: int, m: int, V: float, samples: int = 2000) -> tuple[float, float]:
    ...
    u_guess = np.linspace(0.1, max(V - 0.1, 0.2), int(samples))
    w_guess = np.sqrt(np.clip(V**2 - u_guess**2, 0.0, None))

    
    lhs = (jv(l, u_guess) / u_guess) * jv(l - 1, u_guess)
    rhs = -(kv(l, w_guess) / (w_guess + 1e-12)) * kv(l - 1, w_guess)
    diff = lhs - rhs

    cross_points = np.where(np.diff(np.sign(diff)))[0]
    if len(cross_points) < m:
        raise ValueError(f"LP{l}{m} mode not found under V={V:.2f}")

    u = float(u_guess[cross_points[m - 1]])
...
def parse_lp_name(mode_name: str) -> tuple[int, int]:
    """
    Parse 'LP01' -> (0,1), 'LP12' -> (1,2), 'LP03' -> (0,3).
    """
